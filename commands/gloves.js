    ...
    async execute(interaction) {
        try {
            if (interaction.options.getSubcommand() === '查詢') {
                ...
                const updateName = interaction.options.getString('手套');
                const updatePriceValue = interaction.options.getString('更新價格為');
                const socketNumber = interaction.options.getString('洞數');

                await updatesSheetData(process.env.GOOGLE_SHEET_ID, ENUM_GLOVES_SHEET, updateName, Number(socketNumber), `${updatePriceValue}萬`);
