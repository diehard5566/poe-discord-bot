    ...
    async execute(interaction) {
        try {
            if ( interaction.options.getSubcommand() === '查詢') {
                ...
                await updatesSheetData(process.env.GOOGLE_SHEET_ID, ENUM_AMULET_SHEET, search, Number(socketNumber), newValue);

                await interaction.editReply({ content: `已更新 "${search}" - "${socketNumber} 洞" 的市集價格為：${newValue}`, ephemeral: true });
            }
        } catch (error) {
            console.log(error);
        }
